...
function logger($type, $this) {
    ...
    if ($type == "int") {
        echo (int) $this . PHP_EOL;
        $payload = True;
    } else {
        if ($type == "str") {
            echo (string) $this . PHP_EOL;
            $payload = True;
        } else {
            if ($type == "bool") {
                $payload = $this;
