function main() {
    ...
    var name: string;
    var mode: number, var_number: number;
    ...
    if (mode == 1 || mode == 2) {
        if (mode == 1) {
            ...
            // Kolay Oyun Modu
            var index: number, index2: number, think: number;
            
            index = 0;
            index2 = 0;
            sync = true;
            var_number = rand(true);
                    ...
                    if (think == var_number) {
                        ...
                        if (index == 10) {
                            ...
                            if (think > var_number) {
                                ...
                                if (var_number - think > 25) {
                                    
                                    // Tahmin, sayıdan uzak ise bunu gönder
                                    logger("str", plugins("cold", ""));
                                } else {
                                    if (var_number - think > 15) {
        ...
        if (mode == 3) {
            ...
            var_number = 0;
            do {
                logger("str", plugins("info", String(var_number)));
                delay(4);
...
function logger(var_type: string, var_this: string): boolean {
    var payload: boolean;
    ...
    if (var_type == "int") {
        output(parseInt(var_this));
        payload = true;
    } else {
        if (var_type == "str") {
            output(String(var_this));
            payload = true;
        } else {
            if (var_type == "bool") {
                payload = var_this;
...
function rand(var_type: boolean): number {
    var payload: number, opt: number, num: number;
    var index: number[] = new Array(101);
