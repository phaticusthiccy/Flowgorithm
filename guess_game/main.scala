object MyProgram {
    def main(args: Array[String]) {
        var sync, find : Boolean = false
        var name : String = ""
        var mode, number : Int = 0
    ...
    def logger(var_type : String, var_this : String)  = {
        var payload : Boolean = false
        ...
        if (var_type == "int") {
            println(var_this.toInt)
            payload = true
        } else {
            if (var_type == "str") {
                println(var_this.toString)
                payload = true
            } else {
                if (var_type == "bool") {
                    payload = var_this
    ...
    def namecorrection(name : String)  = {
        var payload : Boolean = false
        
        if (name.length < 3) {
            logger("str", "İsminiz 3 kelimeden kısa olmamalı." + 13.toChar + "Lütfen programı baştan başlatın.")
            exit()
        } else {
            payload = true
        }
    ...
    def plugins(var_type : String, arg : String)  = {
        ...
        if (var_type == "ask name") {
            str(0) = "Oyuna Hoşgeldin Yabancı!" + 13.toChar + "Lütfen Adını Gir:"
            str(1) = "Hey, Merhaba Yabacı :)" + 13.toChar + "Lütfen İsmini Yazar mısın?"
            str(2) = "Yeni bir oyuncu. Yaşasınn!" + 13.toChar + "Adını söylermisin lütfen.."
            str(3) = "Oyunda Yeni Bir Katılımcı Var!" + 13.toChar + "Lütfen İsmini Gir:"
            str(4) = "İşte yeni bir oyuncu daha!" + 13.toChar + "Sana nasıl hitap etmemi istersin?"
            str(5) = "Seni daha önce buralarda görmedim.." + 13.toChar + "Oyuna başlaman için ismine ihtiyacım var. Lütfen ismini söyle."
            str(6) = "Oyuna Hoşgeldin!" + 13.toChar + "Birazdan aşağıdaki giriş bölümü açılacak. Lütfen oraya adını yaz."
            payload = str((Math.random * 7).toInt)
        } else {
            if (var_type == "after name") {
                str(0) = "Merhaba " + arg + 13.toChar + "Oyunun kuralları şunlardır:" + 13.toChar + "1) Sistem 0-100 arasında sayı seçer." + 13.toChar + "2) Sayıyı tahmin etmeye çalışırsın" + 13.toChar + "3) Toplam 10 adet başarısız denemeden sonra oyun biter." + 13.toChar + "4) Sayı bulunursa oyun kazanılır!"
                payload = str(0)
            } else {
                if (var_type == "game mode") {
                    str(0) = "Lütfen Oyun Modunu Seç" + 13.toChar + 13.toChar + "1) Kolay (Sıcak-Soğuk Mantığı)" + 13.toChar + "2) Normal (Yönergeler Yok)" + 13.toChar + "3) İşleyiş Hakkında Bilgi Al"
                    payload = str(0)
                } else {
                    if (var_type == "wmode") {
                        str(0) = "Lütfen Sadece 1 veya 2 Yazın"
                        str(1) = "Yanlış Seçim! Sadece 1 veya 2 yazın!"
                        payload = str((Math.random * 2).toInt)
                    } else {
                        if (var_type == "get num") {
                            ...
                            payload = str((Math.random * 3).toInt)
                        } else {
                            if (var_type == "think") {
                                str(0) = "Tahminin?"
                                ...
                                payload = str((Math.random * 9).toInt)
                            } else {
                                if (var_type == "wthink") {
                                    ...
                                    if (var_type == "sthink") {
                                        ...
                                        payload = str((Math.random * 5).toInt)
                                    } else {
                                        if (var_type == "win") {
                                            str(0) = "[+] Kazandın!!!" + 13.toChar + "İşte Buuu! Nasılda tahmin ettin *o*" + 13.toChar + 13.toChar + "Bir sonraki oyunda görüşmek üzere.."
                                            str(1) = "[+] Tebrikler " + arg + "." + 13.toChar + "Oyunu başarılı bir şekilde bitirdin. Tahmininde haklı çıktın." + 13.toChar + "Bir sonraki oyununda başarılar."
                                            str(2) = "[+] Seni tebrik ederim. Karşımda 6. Hissi çok kuvvetli biri var. Nasıl olurda kaybedeceğini düşündüm ki :)" + 13.toChar + "Bir oyuna daha var mısın.."
                                            str(3) = "[+] Vay bee, gerçektende tahmin etmeyi başardın.. Seninle gurur duyuyorum! Kazandığın için çok mutluyum." + 13.toChar + "Sonraki oyunlarda görüşmek dileğiyle.."
                                            str(4) = "[+] İyi iş çıkardın " + arg + 13.toChar + "Bu oyunun hakkını veriyorsun! Ama biliyorumki birdahaki sefere bu kadar kolay kazanamazsın \\'_'/"
                                            payload = str((Math.random * 5).toInt)
                                        } else {
                                            if (var_type == "swin") {
                                                ...
                                                if (var_type == "lose") {
                                                    ...
                                                    if (var_type == "cold") {
                                                        str(0) = "↘                ↙" + 13.toChar + " Soğuk" + 13.toChar + "↗                ↖"
                                                        payload = str(0)
                                                    } else {
                                                        if (var_type == "warm") {
                                                            str(0) = "↘                ↙" + 13.toChar + "  Ilık" + 13.toChar + "↗                ↖"
                                                            payload = str(0)
                                                        } else {
                                                            if (var_type == "hot") {
                                                                str(0) = "↘                ↙" + 13.toChar + " Sıcak" + 13.toChar + "↗                ↖"
                                                                payload = str(0)
                                                            } else {
                                                                if (var_type == "info") {
    ...
    def rand(var_type : Boolean)  = {
        var payload, opt, num : Int = 0
        var index = new Array[Int](101)
